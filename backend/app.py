...
app = Flask(__name__)
# Configure secrets from .env (used for sessions/JWT if enabled)
...
def plan_trip():
    ...
    if selected_hotel_details:
         # If hotel is a local fallback suggestion, keep destination center to avoid wrong geocodes
    ...
    # --- Transport Suggestion (clearer, people-aware heuristic) ---
    try:
